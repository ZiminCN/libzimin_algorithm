...
template <typename DataFormat> class RingBuffer
{
	...
	bool write(std::vector<DataFormat> &data)
	{
		...
		if ((this->ringbuffer.write_partition.tail_index -
		     this->ringbuffer.write_partition.base_index) >= this->ringbuffer.buffer_size) {
			this->ringbuffer.write_partition.base_index += this->ringbuffer.buffer_size;
		}
	...
	void force_write(std::vector<DataFormat> &data)
	{
		(void)data;
	}
	bool read(std::vector<DataFormat> &data)
	{
		...
		if ((this->ringbuffer.read_partition.tail_index -
		     this->ringbuffer.read_partition.base_index) >= this->ringbuffer.buffer_size) {
			this->ringbuffer.read_partition.base_index += this->ringbuffer.buffer_size;
		}
	...
	{
		std::cout << "Ring buffer real size: " << this->_ring_buffer.size() << std::endl;
		...
		std::cout << "Read size is : "
			  << static_cast<int>(this->ringbuffer.read_partition.size) << std::endl;
		...
		std::cout << "Write size is : "
			  << static_cast<int>(this->ringbuffer.write_partition.size) << std::endl;
